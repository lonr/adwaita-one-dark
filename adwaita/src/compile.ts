...
export async function compileColors(): Promise<void> {
  return compileScss([
    new URL('colors/gtk.scss', srcDir),
    new URL('colors/gtk-dark.scss', srcDir),
  ]);
}
...
export async function compileShell(): Promise<void> {
  await Promise.all([
    compileScss(
      [new URL('gnome-shell/gnome-shell.scss', srcDir)],
      fixResourceUrls,
    ),
    compileScss(
      [new URL('gnome-shell/gnome-shell-dark.scss', srcDir)],
      fixResourceUrls,
    ),
    compileScss(
      [new URL('gnome-shell/gnome-shell-light.scss', srcDir)],
      fixResourceUrls,
    ),
  ]);
}
  ...
  csstree.walk(ast, (node) => {
    if (node.type === 'Url') {
      node.value = node.value.replace(
        /^resource:\/\/\/org\/gnome\/shell\/theme\//,
        '',
      );
    }
  });
